...
workers.validateCheckData = function (originalCheckData) {
    ...
    originalCheckData.url = typeof (originalCheckData.url) == 'string' && originalCheckData.id.trim().length > 0 ? originalCheckData.url.trim() : false;
    originalCheckData.method = typeof (originalCheckData.method) == 'string' && ['post', 'get', 'put', 'delete'].indexOf(originalCheckData.method) > -1 ? originalCheckData.method : false;
    originalCheckData.successCodes = typeof (originalCheckData.successCodes) == 'object' && originalCheckData.successCodes instanceof Array && originalCheckData.successCodes.length > 0 ? originalCheckData.successCodes : false;
    originalCheckData.timeoutSeconds = typeof (originalCheckData.timeoutSeconds) == 'number' && originalCheckData.timeoutSeconds % 1 === 0 && originalCheckData.timeoutSeconds >= 1 && originalCheckData.timeoutSeconds <= 5 ? originalCheckData.timeoutSeconds : false;


    // Set the keys that may have not been set if the workers have never seen this check before
...
workers.performCheck = function (originalCheckData) {
    ...
    var path = parsedUrl.path; // using path and not pathname because we want the complete query string
    ...
    var req = _moduleToUse.request(requestDetails, function (res) {
        ...
        // Update the checkOutcome and pass the data string
    ...
    // Bind to the error so that it does not get thrown
...
// Process the check outcome and update the check data as needed and then trigger an alert to the user if needed
...
workers.alertUsersToStatusChange = function (newCheckData) {
    ...
    helpers.sendTwilioSms(newCheckData.userPhone, msg, function (err) {
        if (!err) {
            ...
            debug('Error: could not sms alert to user who had a state change in their check');
        }
    });
}
