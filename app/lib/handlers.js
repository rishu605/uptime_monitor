...
handlers.index = function (data, callback) {
    ...
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head,description': 'We offer free simple monitoring for HTTP/HTTPS sites of all kinds.',
            'body.class': 'index'
        };
...
handlers._users.get = function (data, callback) {
    ...
    if (phone) {
        ...
        // Verify that the goven token is valid for the phone number
...
// @TODO only let an authenticated user udpate their own object
                ...
                _data.read('users', phone, function (err, userData) {
                    if (!err && data) {
                        _data.delete('users', phone, function (err) {
                            if (!err) {
                                // Delete each of the checks associated with th user
                                        ...
                                        _data.delete('checks', checkId, function (err) {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionErrors) {
                                                    ...
                                                    callback(500, {
                                                        'Error': 'Errors encountered while deleting the user\'s checks. All checks may have not been deleted successfully'
                                                    });
                                                }
                                            }
                                        });
...
handlers._tokens.post = function (data, callback) {
    ...
    var phone = typeof (data.payload.phone) === 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone.trim() : false;
    var password = typeof (data.payload.password) === 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
    performance.mark('inputs valiated');
    if (phone && password) {
        // Lookup for the user who matches the phone number
        performance.mark('beginning user lookup');
        _data.read('users', phone, function (err, userData) {
            performance.mark('user lookup complete');
            if (!err && userData) {
                performance.mark('beginning performance hashing');
                // Hash the password and compare it with the password stored in the user object
                var hashedPassword = helpers.hash(password);
                ...
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    _data.create('tokens', tokenId, tokenObject, function (err) {
                        ...
                        _performance.measure('User lookup', 'beginnning user lookup', 'user lookup complete');
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            ...
                            // Verify that the URL given has DNS entrie (and therefore can be resolved)
    ...
    if (id) {
        ...
        _data.read('checks', id, function (err, checkData) {
            if (!err && checkData) {
                ...
                // Get the token from the headers
                var token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
                // Verify that the goven token is valid and belongs to the user who created the check
...
handlers._checks.delete = function (data, callback) {
    ...
    if (id) {
        ...
        // Look up the check to be deleted
        _data.read('checks', id, function (err, checkData) {
            if (!err && checkData) {
