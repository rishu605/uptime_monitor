...
cli.responders.help = function(){
    var commands = {
        ...
        'stats': 'get statistics on the underlying OS and resoiurce utilization',
        'list users': 'Show a list of all the registered aka undeleted users in the system',
    ...
    // Sow each command, followed by its explanation in white and yellow respectively
...
cli.centered = function(str){
    ...
    // Get the vailable screen sze
    var width = process.stdout.columnsl

    // Calculating the left padding there should be
            ...
            userIds.forEach(function(userId){
                _data.read('users', userId, function(err, userData){
                    if(!err && userData){
                        ...
                        var numberOdChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array && userData.checks.length > 0 ? userData.checks.length : 0;
                        line += numberOdChecks;
                        console.log(line);
                        cli.verticalSpace();
                    }
...
// Init script
cli.init = function(){
    // Send the start message to the console, in blue colour
